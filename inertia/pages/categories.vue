            ...
            >
              ...
              <td class="py-3 w-16 flex items-center justify-center">
                <MenuDropdown :icon="Ellipsis">
                  <MenuItem>
                    <button
                      @click.stop="openModal('view', category)"
                  ...
                  <MenuItem>
                    <button
                      @click.stop="openModal('edit', category)"
                      class="flex justify-start items-center gap-x-2 px-4 py-3 w-full hover:bg-slate-100 text-slate-600 hover:text-slate-950"
                    >
...
const headers = ref(['Name', 'Description', 'Tours', 'Actions'])

const openModal = (mode: string, category: Category | null = null) => {
...
const saveCategory = ({ mode, data }: { data: any; mode: any }) => {
  mode === 'edit' ? router.post('/dashboard/categories', data) : false
  closeModal()
}
